...
{	
	...
	{	
		...
		if(isset($_SESSION)){
			...
			$_SESSION['captcha_code'] = $this->result;
		}
	}
	...
	protected function random($characters=4,$letters = '0123456789'){
		...
		$smbl = '+x';
		$res=0;
		$str1 = substr($letters1, mt_rand(0, strlen($letters1)-1), 1);
		$str2 = substr($letters2, mt_rand(0, strlen($letters2)-1), 1);
		$symbl = substr($smbl, mt_rand(0, strlen($smbl)-1), 1);
		$str=$str1." ".$symbl." ".$str2;
		if($symbl=="+")	$res=(int)$str1 + (int)$str2;
		elseif($symbl=="x")	$res=(int)$str1 * (int)$str2;
		$this->result=$res;
		return $str;
	}	
